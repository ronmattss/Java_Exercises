...
public class Parser {
    ...
    private String exp; // the expression string
    private int expIdx; // the current index of the expression
    ...
    private void getToken() {
        tokType = NONE;
        token = "";
        // CHeck if expression is finished
        ...
        while (expIdx < exp.length() && Character.isWhitespace(exp.charAt((expIdx))))
            ++expIdx;
        //if WhiteSpace is the last character
        if (expIdx == exp.length()) {
            token = EOE;
            return;
        }
